...
while (list($champ, $proprietes) = each($fields))
{
  ...
  // ou alors dans le cas de l'email obligatoire pour self_adherent
  if ($proprietes_arr["not_null"]==1 || $fieldname=="email_adh")
...
if (isset($_POST["valid"])) {
  ...
  while (list($champ, $proprietes) = each($fields)) {
	...
	if($proprietes_arr["type"]=="date" && $post_value!="") {
	  ...
	  if ($post_value=="") {
	    $error_detected .= "<LI>"._T("- Vous ne pouvez pas créer votre fiche en ligne sans déclarer une adresse de courriel !")."</LI>";
	  }
	  ...
	  if (strlen($post_value)<4)
	    ...
	    $requete = "SELECT id_adh FROM ".PREFIX_DB."adherents
    ...
    if ($id_adh=="") { 
      $requete = "SELECT max(id_adh) AS max FROM ".PREFIX_DB."adherents";
...
<FORM action="ajouter_adherent.php" method="post" enctype="multipart/form-data">
  ...
  <BLOCKQUOTE>
    <DIV align="center">
	  ...
	  <TD colspan="3">
	    <INPUT type="radio" name="titre_adh" value="3"<? isChecked($titre_adh,"3") ?> <? echo $disabled_field; ?>> 
            <? echo _T("Mademoiselle"); ?>&nbsp;&nbsp;
	    <INPUT type="radio" name="titre_adh" value="2"<? isChecked($titre_adh,"2") ?> <? echo $disabled_field; ?>> 
            <? echo _T("Madame"); ?>&nbsp;&nbsp;
	    <INPUT type="radio" name="titre_adh" value="1"<? isChecked($titre_adh,"1") ?> <? echo $disabled_field; ?>> 
            <? echo _T("Monsieur"); ?>&nbsp;&nbsp;
	  </TD> 
        </TR> 
        <TR> 
  	  ...
  	  <TD>
            <INPUT type="text" name="nom_adh" value="<? echo $nom_adh; ?>" maxlength="<? echo $nom_adh_len; ?>" <? echo $disabled_field; ?>>
          </TD> 
  	...
  	elseif (file_exists(WEB_ROOT . "photos/tn_" . $id_adh . ".png"))
  	  ...
  	  <TD>
            <INPUT type="text" name="prenom_adh" value="<? echo $prenom_adh; ?>" maxlength="<? echo $prenom_adh_len; ?>" <? echo $disabled_field; ?>>
          </TD> 
        </TR>						   
        <TR> 
        ...
        <TR>
  	  <TH <? echo $activite_adh_req ?> id="libelle">
  	    <? echo _T("Compte :"); ?>
  	  </TH> 
  	  <TD>
  	    <SELECT name="activite_adh">
  	      <OPTION value="1"<? isSelected($activite_adh,"1") ?>>
  	        <? echo _T("Actif"); ?>
  	      </OPTION>
  	      <OPTION value="0"<? isSelected($activite_adh,"0") ?>>
  	        <? echo _T("Inactif"); ?>
  	      </OPTION>
  	    </SELECT>
  	  </TD>
        ...
        <TR> 
  	  <TH <? echo $id_statut_req ?> id="libelle">
  	    <? echo _T("Statut :"); ?>
          </TH> 
  	  <TD>
  	    <SELECT name="id_statut">
  	      <?
  	        $requete = "SELECT * FROM ".PREFIX_DB."statuts
  		 	    ORDER BY priorite_statut";
                $result = &$DB->Execute($requete);
  	        while (!$result->EOF) {
              ?>
  	      <OPTION value="<? echo $result->fields["id_statut"] ?>"<? isSelected($id_statut,$result->fields["id_statut"]) ?>>
  	        <? echo _T($result->fields["libelle_statut"]); ?>
  	      </OPTION>
  	      <?
  	          $result->MoveNext();
  	        }
  	  ...
  	  <TD>
  	    <input type="checkbox" name="bool_admin_adh" value="1"<? isChecked($bool_admin_adh,"1") ?>>
  	  </TD> 
  	  <TH id="header" colspan="2">&nbsp;</TH>
        </TR> 
        <TR> 
  	  ...
  	  <TD>
            <INPUT type="checkbox" name="bool_exempt_adh" value="1"<? isChecked($bool_exempt_adh,"1") ?>>
  	  </TD> 
  	  <TH id="header" colspan="2">&nbsp;</TH>
        </TR>
        <? } ?>
  	  ...
  	  <TD>
  	    <INPUT type="text" name="url_adh" value="<? echo $url_adh; ?>" maxlength="<? echo $url_adh_len; ?>" size="30">
  	  </TD> 
  	  ...
  	  <TD>
  	    <INPUT type="text" name="mdp_adh" value="<? echo $mdp_adh; ?>" maxlength="<? echo $mdp_adh_len; ?>"><BR>
  	    <DIV class="exemple">
  	      <? echo _T("(au moins 4 caractères)"); ?>
  	    </DIV>
  	  </TD> 
        ...
        <TR> 
  	  <TH id="libelle">
  	    <? echo _T("Envoi de mail :"); ?><BR>&nbsp;
          </TH> 
  	  <TD colspan="3">
  	    <INPUT type="checkbox" name="mail_confirm" value="1" <? if ($id_adh=="") echo "CHECKED"; ?>><BR>
  	    <DIV class="exemple">
  	      <? echo _T("(l'adhérent recevra son identifiant et son mot de passe par mail, s'il a une adresse.)"); ?>
            </DIV>
  	  </TD> 
        </TR> 
        <TR> 
  	  <TH id="libelle">
  	    <? echo _T("Date de création :"); ?><BR>&nbsp;
          </TH> 
  	  <TD colspan="3">
  	    <INPUT type="text" name="date_crea_adh" value="<? echo $date_crea_adh; ?>" maxlength="10"><BR>
  	    <DIV class="exemple">
  	      <? echo _T("(format jj/mm/aaaa)"); ?>
  	    </DIV>
  	  </TD> 
        ...
        <TR> 
  	  <TH id="libelle" <? echo $info_adh_req ?>>
  	    <? echo _T("Autres informations (admin) :"); ?>
  	  </TH> 
  	  <TD colspan="3">
  	    <TEXTAREA name="info_adh" cols="61" rows="6">
  	      <? echo $info_adh; ?>
  	    </TEXTAREA><BR>
  	    <DIV class="exemple">
  	      <? echo _T("Ce commentaire n'est visible que par les administrateurs."); ?>
            </DIV>
  	  </TD> 
        </TR> 
        <? } ?>
  	  ...
  	  <TD colspan="3">
  	    ...
  	    <? if ($_SESSION["admin_status"]!=0) { ?>	
            <BR>
  	    <DIV class="exemple">
  	      <? echo _T("Ce commentaire est réservé à l'adhérent."); ?>
  	    </DIV>
  	    <? } ?>	
  	  </TD> 
        </TR> 
