 ...
 * GET /api/v1/members
 *
 * List members with pagination. Requires staff access or higher.
 * Query params: page (default 1), per_page (default 20, max 100)
 *
 * @author Johan Cwiklinski <johan@x-tnd.be>
 */
class ListMembersAction extends AbstractApiController
{
    /**
     * Handle GET /api/v1/members
     *
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        $repo = new Members($filters);
        $members = $repo->getMembersList(true, null, true, false, !$login->isAdmin() && !$login->isSuperAdmin());

        $data = array_map(
            static fn(Adherent $m) => MemberDto::fromAdherent($m)->toArray(),
            $members
        );
