...
{if !$head_redirect}
		...
		<table>
			<tr>
				<th {if $required.mdp_adh eq 1}style="color: #FF0000;"{/if} class="libelle">{_T string="Password:"}<br/>&nbsp;</th>
				<td colspan="3">
					<input type="password" name="mdp_adh" value="" maxlength="20"/>
					<div class="exemple">{_T string="(at least 4 characters)"}</div><br/>
					<input type="password" name="mdp_adh2" value="" maxlength="20"/>
					<div class="exemple">{_T string="(Confirmation)"}</div><br/>
				</td>
			</tr>
		</table>
		<div>
