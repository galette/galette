...
{foreach from=$dyn_fields item=field name=alldyn}
    ...
    {if isset($field.no_data)}
                        ...
                        <img src="{base_url}/{$template_subdir}images/icon-empty.png" alt="" border="0" width="16" height="16"/>
    {else}
                        <a href="editer_champ.php?form={$form_name}&amp;id={$field.id}"><img src="{base_url}/{$template_subdir}images/icon-edit.png" alt="{_T string="Edit '%s' field" pattern="/%s/" replace=$field.name}" title="{_T string="Edit '%s' field" pattern="/%s/" replace=$field.name}" width="16" height="16"/></a>
                        <a href="traduire_libelles.php?text_orig={$field.name|escape}"><img src="{base_url}/{$template_subdir}images/icon-i18n.png" alt="{_T string="Edit '%s' field" pattern="/%s/" replace=$field.name}" title="{_T string="Translate '%s' field" pattern="/%s/" replace=$field.name}" width="16" height="16"/></a>
    {/if}
    ...
    {else}
                        <a href="configurer_fiches.php?form={$form_name}&amp;up={$field.id}">
                            <img src="{base_url}/{$template_subdir}images/icon-up.png" alt="{_T string="Send up '%s' field" pattern="/%s/" replace=$field.name}" title="{_T string="Send up '%s' field" pattern="/%s/" replace=$field.name}" width="9" height="8"/>
                        </a>
    {/if}
    ...
    {else}
                        <a href="configurer_fiches.php?form={$form_name}&amp;down={$field.id}">
                            <img src="{base_url}/{$template_subdir}images/icon-down.png" alt="{_T string="Send down '%s' field" pattern="/%s/" replace=$field.name}" title="{_T string="Send down '%s' field" pattern="/%s/" replace=$field.name}" width="9" height="8"/>
                        </a>
    {/if}
                    </td>
                </tr>
