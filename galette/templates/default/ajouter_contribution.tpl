		<h1 class="titre">{_T string="Contribution card"} ({if $data.id_cotis != ""}{_T string="modification"}{else}{_T string="creation"}{/if})</h1>
...
{/if}
		...
		<div align="center">
			<table border="0" id="input-table">
				<tr>
					<th {if $required.id_adh eq 1}style="color: #FF0000;"{/if} class="libelle">{_T string="Contributor:"}</th>
					<td>
						<select name="id_adh">
					...
					<th {if $required.id_type_cotis eq 1}style="color: #FF0000;"{/if} class="libelle">{_T string="Contribution type:"}</th>
					<td>
				...
				<tr>
					<th {if $required.montant_cotis eq 1}style="color: #FF0000;"{/if} class="libelle">{_T string="Amount:"}</th>
					<td {if !$data.trans_id}colspan="3"{/if}><input type="text" name="montant_cotis" value="{$data.montant_cotis}" maxlength="10"/></td>
					{if $data.trans_id}
					<th class="libelle">{_T string="Transaction number:"}</th>
				...
				<tr>
					...
					<td>
						...
						<div class="exemple">{_T string="months"}</div>
						{else}
				...
				<tr>
					<th {if $required.info_cotis eq 1}style="color: #FF0000;"{/if} class="libelle">{_T string="Comments:"}</th>
...
{include file="display_dynamic_fields.tpl" is_form=true}
				...
				<tr>
					<th align="center" colspan="4"><br/><input type="submit" class="submit" value="{_T string="Continue"}"/></th>
				</tr>
				{/if} {* $type_selected eq 1 *}
			</table>
		</div>
