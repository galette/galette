...
{foreach item=ex from=$written}
                <li><a href="{path_for name="getCsv" data=["type" => {_T string="export" catalog="routes"}, "file" => $ex.name]}">{$ex.name} ({$ex.file})</a></li>
{/foreach}
            </ul>
        </div>
{/if}
{if $existing|@count gt 0}
    ...
    {foreach item=export from=$existing name=existing_list}
                            <tr class="{if $smarty.foreach.existing_list.iteration % 2 eq 0}even{else}odd{/if}">
                                <td >
                                    <a href="{path_for name="getCsv" data=["type" => {_T string="export" catalog="routes"}, "file" => $export.name]}">{$export.name}</a>
                                </td>
                                <td>
                                    {$export.date}
                                </td>
                                ...
                                <td class="actions_row">
                                    <a href="{path_for name="removeCsv" data=["type" => {_T string="export" catalog="routes"}, "file" => $export.name]}" title="{_T string="Remove '%file' from disk" pattern="/%file/" replace=$export.name}"><img src="{$template_subdir}images/delete.png" alt="{_T string="Delete"}"/></a>
                                </td>
                            </tr>
    {/foreach}
                        </tbody>
