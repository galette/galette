    ...
    <tbody>
	  ...
	  <td class="listing center actions_row">
	    <a href="editer_intitules.php?class={$class}&amp;id={$entry.id}">
	      <img src="{$template_subdir}images/icon-edit.png" alt="{_T string="Edit '%s' field"|regex_replace:"/%s/":$class}" title="{_T string="Edit '%s' field"|regex_replace:"/%s/":$class}" width="16" height="16"/>
	    </a>
	    <a onclick="return confirm('{_T string="Do you really want to delete this category?"|escape:"javascript"}')" href="editer_intitules.php?class={$class}&amp;del={$entry.id}">
	      <img src="{$template_subdir}images/icon-trash.png" alt="{_T string="Delete '%s' field"|regex_replace:"/%s/":$class}" title="{_T string="Delete '%s' field"|regex_replace:"/%s/":$class}" width="16" height="16" />
	    </a>
	  </td>
	</tr>
      {/foreach}
