        ...
        if ( isset($_POST['valid']) ) {
               ...
               (!isset($_POST['field_name']) || $_POST['field_name'] == '')
                ...
                if ($idx != false) {
                    ...
                    if ($field_type != $field_type_separator
                        ...
                        header('location: editer_champ.php?form=' . $form_name .
                            '&id=' . $field_id);
                    }
                    if ( $field_name != '' ) {
            ...
            if ( $action != "" ) {
                ...
                if ( $res != false && !$res->EOF ) {
                    ...
                    $query_list = array();
                    if ( $action == 'del' ) {
                        $query_list[] = 'UPDATE $field_types_table SET field_index=field_index-1 WHERE field_index > ' . $old_rank . ' AND field_form=' . $quoted_form_name;
                        $query_list[] = "DELETE FROM $fields_table WHERE field_id=$field_id AND field_form=$quoted_form_name";
                        $query_list[] = "DELETE FROM $field_types_table WHERE field_id=$field_id AND field_form=$quoted_form_name";
                        ...
                        $query = "SELECT field_name FROM $field_types_table WHERE field_id=$field_id";
