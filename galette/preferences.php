...
$is_super_admin = ( $_SESSION["logged_username"] == PREF_ADMIN_LOGIN ) ? true : false;
...
if ( isset($_POST['valid']) && $_POST['valid'] == "1" ) {
            ...
            case 'pref_admin_login':
                if( $is_super_admin ) {
                    if ( strlen($value) < 4 ) {
                        ...
                        $requete2 = "SELECT id_adh FROM " . PREFIX_DB .
    ...
    if ( count($error_detected) == 0 ) {
        ...
        while ( list($champ, $valeur) = each($insert_values) ) {
            if ( $is_super_admin || (!$is_super_admin && ($champ != 'pref_admin_pass' && $champ != 'pref_admin_login')) ) {
                if( ($champ == "pref_admin_pass" && $_POST['pref_admin_pass']!= '')
                    | ($champ != "pref_admin_pass")) {
                    $valeur = stripslashes($valeur);
                    $requete = "UPDATE " . PREFIX_DB . "preferences set val_pref=" .
                        $DB->qstr($valeur) . " WHERE nom_pref=" .
                        $DB->qstr($champ) . ";\n";
                    $DB->Execute($requete);
                }
            }
...
$tpl->assign('is_super_admin', $is_super_admin);
