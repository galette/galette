 ...
 */
class Html extends GaletteTestCase
{
    /**
     * Dangerous markup never survives clean()
     */
    public function testClean(): void
    {
        $this->assertSame('<p>Hello</p>', \Galette\Util\Html::clean('<p>Hello</p>'));
        $this->assertStringNotContainsString(
            '<script',
            \Galette\Util\Html::clean('<script>alert(1)</script>')
        );
        $this->assertStringNotContainsString(
            'javascript:',
            \Galette\Util\Html::clean('<a href="javascript:alert(1)">x</a>')
        );
        //a link to an allowed scheme is kept
        $this->assertStringContainsString(
            'https://galette.eu',
            \Galette\Util\Html::clean('<a href="https://galette.eu">x</a>')
        );
    }
    ...
    public static function textConversionProvider(): array
    {
        return [
            ...
            [
                ...
                'expected' => "A first paragraph, with a [link](https://galette.eu \"Link title\")\n\nAnd another paragraph."
            ],
            [
            ...
            [
                ...
                'expected' => "Any text ![Alternative text](http://galette.eu/logo)"
            ],
            [
                'input' => '<div>Any text <img src="hTtPs://galette.eu/logo" alt="Alternative text"/></div>',
                'expected' => "Any text ![Alternative text](hTtPs://galette.eu/logo)"
            ],
            [
    ...
    public function testTextConversion(string $input, string $expected): void
    {
        $this->assertSame(
            $expected,
            \Galette\Util\Html::convertToText($input)
        );
    }
}
