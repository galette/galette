...
E2E_ADMIN_USER=admin E2E_ADMIN_PASS=admin \
  npx playwright test --project mail-queue
```
...
Both queue specs live in the `mail-queue` project, which runs on a single
worker: they share the Mailpit inbox and the global mail preferences, so they
