...
{/if}
		...
		<DIV align="center">
			<TABLE border="0" id="input-table">
				<TR>
					<TH colspan="2" class="header">{_T("General information:")}</TH>
				</TR> 
				<TR> 
					<TH {if $required.pref_nom eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Name (corporate name) of the association:")}</TH>
					<TD><INPUT type="text" name="pref_nom" value="{$pref.pref_nom}" maxlength="190"/></TD>
				</TR>
				<TR>
					<TH class="libelle">{_T("Logo:")}</TH> 
					<TD>
...
{/if}
				...
				<TR>
					<TH {if $required.pref_adresse eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Address:")}</TH> 
				...
				<TR>
					<TH {if $required.pref_cp eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Zip Code:")}</TH>
					<TD><input type="text" name="pref_cp" value="{$pref.pref_cp}" maxlength="10"/></TD>
				</TR>         
				<TR>
					<TH {if $required.pref_ville eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("City:")}</TH> 
					<TD><INPUT type="text" name="pref_ville" value="{$pref.pref_ville}" maxlength="100"/></TD>
				</TR>         
				<TR>
					<TH {if $required.pref_pays eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Country:")}</TH> 
					<TD><input type="text" name="pref_pays" value="{$pref.pref_pays}" maxlength="50"/></TD>
				</TR>         
				<TR>
					<TH colspan="2" class="header"><BR/>{_T("Galette's parameters:")}</TH>
				</TR> 
				<TR>
					<TH {if $required.pref_lang eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Default language:")}</TH>
					<TD>
...
{/foreach}
				...
				<TR>
					<TH {if $required.pref_numrows eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Lines / Page:")}</TH>
					<TD>
				...
				<TR>
					<TH {if $required.pref_log eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Logging level:")}</TH>
					<TD>
				...
				<TR>
					<TH {if $required.pref_membership_ext eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Default membership extension:")}</TH>
					<TD>
				...
				<TR>
					<TH {if $required.pref_beg_membership eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Beginning of membership:")}</TH>
					<TD>
				...
				<TR>
					<TH colspan="2" class="header"><BR/>{_T("Mail settings:")}</TH>
				</TR> 
				<TR>
					<TH {if $required.pref_email_nom eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Sender name:")}</TH> 
					<TD><INPUT type="text" name="pref_email_nom" value="{$pref.pref_email_nom}" maxlength="50"/></TD>
				</TR>         
				<TR>
					<TH {if $required.pref_email eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Sender Email:")}</TH> 
					<TD><INPUT type="text" name="pref_email" value="{$pref.pref_email}" maxlength="100" size="30"/></TD>
				</TR>
				<TR>
					<TH {if $required.pref_mail_method eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Emailing method:")}</TH>
					<TD>
						<INPUT type="radio" name="pref_mail_method" value="0" {if $pref.pref_mail_method eq 0}CHECKED="checked"{/if}/> {_T("Emailing disabled")}<BR/>
						<INPUT type="radio" name="pref_mail_method" value="1" {if $pref.pref_mail_method eq 1}CHECKED="checked"{/if}/> {_T("PHP mail() function")}<BR/>
						<INPUT type="radio" name="pref_mail_method" value="2" {if $pref.pref_mail_method eq 2}CHECKED="checked"{/if}/> {_T("Using a SMTP server (slower)")}
					</TD
				</TR>
				<TR>
					<TH {if $required.pref_mail_smtp eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("SMTP server:")}</TH>
					<TD><input type="text" name="pref_mail_smtp" value="{$pref.pref_mail_smtp}" maxlength="100" size="30"/></TD>
				</TR> 
				<TR>
					<TH colspan="2" class="header"><BR/>{_T("Label generation parameters:")}</TH>
				</TR> 
				<TR>
					<TH {if $required.pref_etiq_marges eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Margins:")}</TH> 
					<TD>
				...
				<TR>
					<TH {if $required.pref_etiq_hspace eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Horizontal spacing:")}</TH>
					<TD>
				...
				<TR>
					<TH {if $required.pref_etiq_vspace eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Vertical spacing:")}</TH>
					<TD>
				...
				<TR>
					<TH {if $required.pref_etiq_hsize eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Label width:")}</TH>
					<TD>
				...
				<TR>
					<TH {if $required.pref_etiq_vsize eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Label height:")}</TH>
					<TD>
    				...
    				<TR>
   					<TH {if $required.pref_etiq_cols eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Number of label columns:")}</TH> 
				<TD>
				...
				<TR>
   					<TH {if $required.pref_etiq_rows eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Number of label lines:")}</TH> 
					<TD>
    				...
    				<TR>
   					<TH {if $required.pref_etiq_corps eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Font size:")}</TH> 
					<TD>
   				...
   				<TR>
   					<TH colspan="2" class="header"><BR/>{_T("Admin account (independant of members):")}</TH>
   				</TR> 
   				<TR>
   					<TH {if $required.pref_admin_login eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Username:")}</TH> 
					<TD>
						<INPUT type="text" name="pref_admin_login" value="{$pref.pref_admin_login}" maxlength="20"/>
					</TD>
   				</TR>
				<TR> 
					<TH {if $required.pref_admin_pass eq 1}style="color: #FF0000;"{/if} class="libelle">{_T("Password:")}</TH> 
					<TD>
