		<H1 class="titre">{_T("Profile configuration")}</H1>
...
{foreach from=$dyn_fields item=field}
			<TR>
				...
				<TD class="listing">{$field.size}</TD>
				<TD class="listing center">
					<A onClick="return confirm('{_T("Do you really want to delete this category ? All associated data will be deleted as well.")|escape:"javascript"}')" href="configurer_fiches.php?sup={$field.id}">
			...
			<TR>
				...
				<TD width="60" class="listing left">
					<SELECT name="type_cat">
						<OPTION value="{$category_separator}">{_T("separator")}</OPTION>
						<OPTION value="{$category_text}">{_T("free text")}</OPTION>
						<OPTION value="{$category_field}">{_T("field")}</OPTION>
					</SELECT>
				</TD>
				<TD class="listing">
