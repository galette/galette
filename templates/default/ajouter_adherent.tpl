		<h1 class="titre">{_T string="Member Profile"} ({if $data.id_adh != ""}{_T string="modification"}{else}{_T string="creation"}{/if})</h1>
		...
		<div class="bigtable">
			<fieldset class="cssform">
				...
				<p>
					...
					<img src="picture.php?id_adh={$data.id_adh}&amp;rand={$time}" class="picture" width="{$data.picture_width}" height="{$data.picture_height}" alt="{_T string="Picture"}"/><br/>
...
{if $smarty.session.admin_status eq 1}
				<p>
					...
					<select name="activite_adh" id="activite_adh" {$disabled.activite_adh}>
						<option value="1" {if $data.activite_adh eq 1}selected="selected"{/if}>{_T string="Active"}</option>
						<option value="0" {if $data.activite_adh eq 0}selected="selected"{/if}>{_T string="Inactive"}</option>
					</select>
				</p>
				<p>
...
{if $smarty.session.admin_status eq 1}
				<p>
					...
					<input type="checkbox" name="mail_confirm" id="mail_confirm" value="1" {if $smarty.post.mail_confirm != ""}checked="checked"{/if} {$disabled.send_mail}/>
					<span class="exemple">
